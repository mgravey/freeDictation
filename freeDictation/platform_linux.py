...
class FreeDictationApp(FreeDictationAppBase):
    def __init__(self):
       ...
       # Start loading the model
        ...
        # Initialize the icon, but do not start it in a background thread
        self.icon = pystray.Icon("FreeDictation", icon=self.create_image())
        # Build menus after initializing languages
        self.build_menus()
        # Start key event listener in a separate thread
        self.start_key_listener()
        # Run the icon explicitly on the main thread
        self.icon.run()
    ...
    def update_device_checkmarks(self):
        for item in self.device_menu_items:
            item.checked = item.device_index == self.INPUT_DEVICE_INDEX

    def on_select_model(self, item):
        self.select_model(item.model_name)
        # Rebuild the menus to update checkmarks
        self.build_menus()# platform_linux.py
...
class FreeDictationApp(FreeDictationAppBase):
    ...
    def update_model_checkmarks(self):
        for item in self.model_menu_items:
            item.checked = item.model_name == self.MODEL_NAME
    ...
    def update_language_checkmarks(self):
        for item in self.language_menu_items:
            item.checked = item.language == self.config["preferred_language"]
